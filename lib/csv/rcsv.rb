...
class Rcsv
  ...
  def self.parse(csv_data, options = {}, &block)
    ...
    when :use
      puts "#" * 80
      puts self # CSV::Rcsv
      puts CsvParser::Rcsv.hello("test")
      puts "#" * 80
      # TODO:
      header = self.raw_parse(StringIO.new(csv_data.each_line.first), raw_options).first
      # header = CsvParser::Rcsv.raw_parse(StringIO.new(csv_data.each_line.first), raw_options).first
    ...
    # return CsvParser::Rcsv.raw_parse(csv_data, raw_options, &block)
  end
