...
class LibCsv
  ...
  def self.sample
    ...
    pointer = FFI::MemoryPointer.new :char, CsvParser.size, false
    parser = CsvParser.new pointer
    result = LibCsv.csv_init(parser, 0)
    puts result
  end
...
# LibCsv.sample
    puts "# ========================================================================="
    pointer = FFI::MemoryPointer.new :char, LibCsv::CsvParser.size, false
    parser = LibCsv::CsvParser.new pointer
    result = LibCsv.csv_init(parser, 0)
    puts result
    puts "status: #{parser[:status]}"
    puts "quote_char: #{parser[:quote_char]}"
    puts "delim_char: #{parser[:delim_char]}"
